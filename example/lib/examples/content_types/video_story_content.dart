...
class VideoStoryContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AdvStory(
      storyCount: responseData.length,
      // This example creates only video contents, video sizes are large.
      // Disable story preload.
      preloadStory: false,
      storyBuilder: (storyIndex) {
        return Story(
          contentCount: (responseData[storyIndex]['stories'] as List).length,
          contentBuilder: (contentIndex) {
            String type = (responseData[storyIndex]['stories'] as List)[contentIndex]!['type'];
            String url = (responseData[storyIndex]['stories'] as List)[contentIndex]!['url'];
